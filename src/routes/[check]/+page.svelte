<script lang=ts>
    ...
    //console.log('start');
    //console.log(data.props.post[0].result);
</script>
...
<section class="results">
    {#each data.props.post[0].result as video}
        ...
        <div class="fade-in-bottom faster results_card">
            <span>"{transcript.transcript}"</span>
                <!-- {console.log(transcripts)} -->
            <a
                href={`https://www.youtube.com/watch?v=${video.videoId}&t=${Math.floor(transcript.startMs / 1000)}s`}
                target="_blank"
                rel="noreferrer"
